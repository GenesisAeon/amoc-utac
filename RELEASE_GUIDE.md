...
`amoc-utac` is a standalone UTAC application package — it does not depend
on any other `GenesisAeon/*` package at runtime (it implements its own
UTAC-ODE / CREP logic directly). If a future version adds such a
dependency, pin it with a `>=` lower bound matching the minimum version
that provides the API this package relies on, not an exact (`==`) pin.
